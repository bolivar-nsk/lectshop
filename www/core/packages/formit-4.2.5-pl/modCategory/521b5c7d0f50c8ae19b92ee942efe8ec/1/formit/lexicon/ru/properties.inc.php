...
$_lang['prop_formit.storelocation_desc']                        = 'Если для `store` задано значение true, то это указывает местоположение кэша данных из форм. По умолчанию используется кэш MODX.';
...
$_lang['prop_fico.prioritized_desc']                            = 'Необязательный параметр. Разделенный запятыми список кодов ISO для стран, которые будут перемещать их в приоритетную группу «Частых посетителей» в верхней части раскрывающегося списка. Это может быть использовано для ваших обычно выбранных стран.';
...
$_lang['prop_fiso.useabbr_desc']                                = 'Если 1, будет использовать код страны ISO для значения. Если 0, будет использовать название страны.';
...
$_lang['prop_formit.attachfiles_desc']                          = 'Если true, FormIt добавит все поля описания файла как вложения в электронное письмо.';
